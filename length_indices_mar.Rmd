...
# not run
st <- 
  tbl_mar(db, "ops$einarhj.smb19852016") %>% 
  left_join(lesa_stodvar(db) %>% select(synis_id, reitur, tognumer,
                                        lon = kastad_v_lengd,
                                        lat = kastad_n_breidd)) %>% 
  collect() %>% 
...
copy_to(db, STTAKEN4, temporary = FALSE)
```
...
Length <- 5 # Index for fish length equal to or larger than
db <- src_oracle("mar")
Stations <-  tbl_mar(db, "ops$einarhj.STTAKEN4")

teg <- lesa_tegundir(db) %>% 
  collect(n = Inf) %>% 
  arrange(tegund)
...
nu <-
  st %>% 
  select(synis_id) %>% 
  left_join(lesa_numer(db)) %>% 
  collect(n = Inf)
le <- 
  st %>% 
  select(synis_id) %>% 
  left_join(lesa_lengdir(db)) %>% 
  collect(n = Inf)
st <-
  st %>% 
  collect(n = Inf)
